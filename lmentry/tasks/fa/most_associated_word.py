...
class MostAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="most_associated_word"):
        ...
        self.canonical_template = \
            'از بین کلمات {options}، کدام کلمه بیشترین ارتباط را با "{category}" دارد؟'
        self.second_template = \
            'از بین کلمات {options} کدام کلمه بیشترین ارتباط را با کلمه "{category}" دارد؟'
        self.third_template = \
            'از بین کلمات زیر، کلمه‌ای را که بیشترین ارتباط را با کلمه "{category}" دارد انتخاب کنید - {options}'
