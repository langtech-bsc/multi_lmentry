...
class WordAfter(LMentryTask):
    ...
    def __init__(self, name="word_after"):
        ...
        self.canonical_template = 'In the sentence "{sentence}", which word comes right after the word "{word}"?'
        self.second_template = 'In the sentence "{sentence}", which word immediately succeeds the word "{word}"?'
        self.third_template = 'Which word comes right after "{word}" in the sentence "{sentence}"?'
