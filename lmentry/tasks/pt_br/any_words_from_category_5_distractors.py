...
class AnyWordsFromCategory5Distractors(LMentryTask):
    ...
    def __init__(self, name="any_words_from_category_5_distractors"):
        ...
        self.canonical_template = 'Alguma das palavras {words} é um tipo de {category}? Responda exclusivamente com "sim" ou "não".'
        self.second_template = 'Todas estas palavras {words} representam um tipo de {category}? Responda exclusivamente com "sim" ou "não".'
        self.third_template = 'A lista [{words}] contém exclusivamente tipos de {category}? Responda exclusivamente com "sim" ou "não".'
    ...
    def _create_input(self, template, words, category):
        ...
        if template_id != 2:  # add "and" before the last word
            words = ", ".join(words[:-1]) + f" y {words[-1]}"
        else:
            words = ", ".join(words)
