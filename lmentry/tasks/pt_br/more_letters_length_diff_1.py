...
class MoreLettersLengthDiff1(LMentryTask):
    ...
    def __init__(self, name="more_letters_length_diff_1"):
        ...
        self.canonical_template = 'Qual palavra tem mais letras: "{word1}" ou "{word2}"?'
        self.second_template = 'Qual palavra é mais longa: "{word1}" ou "{word2}"?'
        self.third_template = 'Das palavras "{word1}" e "{word2}", qual tem mais letras?'
        self.all_templates = [self.canonical_template, self.second_template, self.third_template]
        self.parameter_names = ["answer", "distractor"]
        self.num_to_word = dict(
            zip(range(2, 11), ["dois", "três", "quatro", "cinco", "seis", "sete", "oito", "nove", "dez"])
        )
