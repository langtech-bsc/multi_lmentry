...
class LeastAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="least_associated_word"):
        ...
        self.canonical_template = \
            'Welches der Wörter {options} wird am wenigsten mit der Kategorie "{category}" assoziiert?'
        self.second_template = \
            'Welches der Wörter {options}, ist am wenigsten mit der Kategorie "{category}" verwandt?'
        self.third_template = \
            'Wähle aus den folgenden Wörtern das Wort aus, das am wenigsten mit der Kategorie "{category}" assoziiert wird - {options}:'
    ...
    def _create_input(self, template, category, distractors, answer, answer_index, template_id):
        ...
        if template_id != self.non_and_template_id:
            options = ", ".join(options[:-1]) + f" und {options[-1]}"
        else:
