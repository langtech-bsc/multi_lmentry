...
class AnyWordsFromCategory3Distractors(LMentryTask):
    ...
    def __init__(self, name="any_words_from_category_3_distractors"):
        ...
        self.canonical_template = 'Sind irgendwelche der Wörter {words} {category}? Antworte entweder mit "ja" oder "nein". '
        self.second_template = 'Repräsentieren irgendwelche der Wörter {words} {category}? Antworte entweder "ja" oder "nein".'
        self.third_template = 'Enthält die Liste [{words}] {category}? Antworte entweder "ja" oder "nein".'
    ...
    def _create_input(self, template, words, category):
        ...
        if template_id != 2:  # add "and" before the last word
            words = ", ".join(words[:-1]) + f" und {words[-1]}"
        else:
            words = ", ".join(words)
