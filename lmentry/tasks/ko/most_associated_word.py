...
from lmentry.scorers.ko.most_associated_word_scorer import MostAssociatedWordScorer
...
class MostAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="most_associated_word"):
        ...
        self.canonical_template = '{options} 단어 중에서 일반적으로 "{category}"와 가장 연관된 단어는 무엇입니까?'
        self.second_template = '{options} 단어에서 "{category}" 단어와 가장 관련이 있는 단어는 무엇입니까?'
        self.third_template = '다음 단어 중에서 일반적으로 "{category}"와 가장 연관된 단어를 선택하십시오. - {options}:'
    ...
    def create_data(self, num_distractors_range: tuple = (4,), num_examples=1000, seed=None,
        ...
        for template_id, template in enumerate(self.all_templates):
            ...
            while num_template_examples_created < num_examples:
                ...
                # sample distractors
                print(f"SOURCE DATA[DISTRACTOR CATEGORY]: {len(source_data[distractor_category])} - expected: {num_distractors} - category: {distractor_category}")
