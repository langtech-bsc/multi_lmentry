...
from lmentry.scorers.ko.word_after_scorer import WordAfterScorer
...
class WordAfter(LMentryTask):
    ...
    def __init__(self, name="word_after"):
        ...
        self.canonical_template = '문장 "{sentence}"에서 "{word}" 바로 뒤에 오는 단어는 무엇입니까?'
        self.second_template = '문장 "{sentence}"에서 "{word}"의 뒤를 바로 잇는 단어는 무엇입니까?'
        self.third_template = '어떤 단어가 문장 "{sentence}"에서 "{word}" 바로 다음에 옵니까?'
