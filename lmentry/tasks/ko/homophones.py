...
class Homophones(LMentryTask):
    ...
    def __init__(self, name="homophones"):
        super().__init__(name)
        self.canonical_template = '"{query}" 는 "{word1}"와 "{word2}"중에 어떤 단어와 비슷하게 들립니까?'
        self.second_template = '"{word1}"과 "{word2}" 두 단어 중 어느 것이 "{query}"와 더 비슷하게 들립니까?'
        self.third_template = '"{query}"의 동음이의어는 "{word1}"와 "{word2}"중 어느 것입니까?'
