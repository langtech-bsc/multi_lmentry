...
from lmentry.scorers.ko.more_letters_scorer import MoreLettersScorer
...
class MoreLettersLengthDiff1(LMentryTask):
    ...
    def __init__(self, name="more_letters_length_diff_1"):
        ...
        self.canonical_template = '더 많은 글자를 가진 것은 단어 {word1}"과 "{word2}" 중 어느 것입니까?"'
        self.second_template = '"{word1}"과 "{word2}" 중 어느 단어가 더 깁니까?'
        self.third_template = '"{word1}"과 "{word2}" 중 어느 것이 더 많은 글자를 가지고 있습니까?'
        self.all_templates = [self.canonical_template, self.second_template, self.third_template]
        self.parameter_names = ["answer", "distractor"]
        self.num_to_word = dict(
            zip(range(2, 11), ["둘", "셋", "넷", "다섯", "여섯", "일곱", "여덟", "아홉", "열"])
        )
