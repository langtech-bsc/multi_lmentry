...
from lmentry.scorers.ko.rhyming_word_scorer import RhymingWordScorer
...
class RhymingWordOrthographicallySimilar(LMentryTask):
    ...
    def __init__(self, name="rhyming_word_orthographically_similar"):
        super().__init__(name)
        self.canonical_template = '다음 단어 "{query}"는 "{word1}" 와 "{word2}"중에 어느 것과 운율이 있습니까?'
        self.second_template = '주어진 단어 "{query}"와 운율이 느껴지는 것은 "{word1}" 와 "{word2}" 중에서 무엇입니까?'
        self.third_template = '단어 "{word1}"과 "{word2}" 중에서, 어떤 것이 "{query}"와 운율이 느껴집니까?'
    ...
    def create_data(self, num_examples=1000, seed=None,
        ...
        distractors_path = distractors_path or LMENTRY_WORDS_PATH
