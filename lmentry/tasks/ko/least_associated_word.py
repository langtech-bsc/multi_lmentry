...
from lmentry.scorers.ko.least_associated_word_scorer import LeastAssociatedWordScorer
...
class LeastAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="least_associated_word"):
        ...
        self.canonical_template = '{options} 단어 중 일반적으로 "{category}"와 가장 연관성이 낮은 단어는 무엇입니까?'
        self.second_template = '{options} 단어에서 "{category}" 단어와 가장 관련이 적은 단어는 무엇입니까?'
        self.third_template = '다음 단어 중에서 일반적으로 "{category}"와 가장 연관성이 낮은 단어를 선택하십시오. - {options}:'
