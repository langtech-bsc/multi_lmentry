...
from lmentry.scorers.ko.sentence_containing_scorer import SentenceContainingScorer
...
class SentenceContaining(LMentryTask):
    ...
    def __init__(self, name="sentence_containing"):
        super().__init__(name)
        self.canonical_template = '"{word}"가 포함된 문장을 작성하시오:'
        self.second_template = '"{word}"라는 단어를 사용하여 문장을 작성하시오:'
        self.third_template = '"{word}"라는 단어를 포함하는 문장을 작성하시오:'
    ...
    def create_data(self, num_examples=999, seed=None, task_data_path=None):

        # sample num_examples words from the data
        task_words = self.get_task_words()
        print("len(task_words):", len(task_words))
