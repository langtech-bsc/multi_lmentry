...
from lmentry.eu_declension import comitative_singular, possessive_genitive_undetermined
...
class AnyWordsFromCategory5Distractors(LMentryTask):
    ...
    def __init__(self, name="any_words_from_category_5_distractors"):
        ...
        self.canonical_template = '{words} hitzetako baten bat da {category} bat? Erantzun "bai" edo "ez".'
        self.second_template = '{words} hitzetako baten batek adierazten du {category} bat? Erantzun "bai" edo "ez".'
        self.third_template = '[{words}] zerrendak badu {category} bat? Erantzun "bai" edo "ez".'
    ...
    def _create_input(self, template, words, category):
        ...
        if category in eu_abstract_categories:
            category = f"{comitative_singular(category)} zerikusia duen hitz"
        if template_id == 2:
            category = possessive_genitive_undetermined(category)

        # create the string representation of the words
        words = [f"\"{word}\"" for word in words]
        if template_id != 2:  # add "and" before the last word
            words = ", ".join(words[:-1]) + f" eta {words[-1]}"
        else:
            words = ", ".join(words)
    ...
    def create_data(self, num_words_options: tuple = (5,), num_distractors_options: tuple = (5,),
        ...
        with open(RESOURCES_DIR.joinpath("nouns_by_category.json")) as f:
