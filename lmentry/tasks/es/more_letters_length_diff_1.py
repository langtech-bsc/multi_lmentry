...
class MoreLettersLengthDiff1(LMentryTask):
    ...
    def __init__(self, name="more_letters_length_diff_1"):
        ...
        self.canonical_template = '¿Qué palabra tiene más letras: "{word1}" o "{word2}"?'
        self.second_template = '¿Qué palabra es más larga: "{word1}" o "{word2}"?'
        self.third_template = 'De las palabras "{word1}" y "{word2}", ¿cuál tiene más letras?'
        self.all_templates = [self.canonical_template, self.second_template, self.third_template]
        self.parameter_names = ["answer", "distractor"]
        self.num_to_word = dict(
            zip(range(2, 11), ["dos", "tres", "cuatro", "cinco", "seis", "siete", "ocho", "nueve", "diez"])
        )
