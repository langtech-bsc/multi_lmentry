...
class MostAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="most_associated_word"):
        ...
        self.canonical_template = \
            'De las palabras {options}, ¿cuál es la palabra más comúnmente asociada con "{category}"?'
        self.second_template = \
            '¿Qué palabra es la más relacionada con "{category}" entre las palabras {options}?'
        self.third_template = \
            'Entre las siguientes palabras, elige la palabra más comúnmente asociada con "{category}" - {options}:'
    ...
    def _create_input(self, template, category, distractors, answer, answer_index, template_id):
        ...
        if template_id != self.non_and_template_id:
            options = ", ".join(options[:-1]) + f" y {options[-1]}"
        else:
