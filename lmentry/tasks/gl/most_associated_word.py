...
from lmentry.scorers.gl.most_associated_word_scorer import MostAssociatedWordScorer
...
class MostAssociatedWord(LMentryTask):
    ...
    def __init__(self, name="most_associated_word"):
        ...
        self.canonical_template = \
            'Das palabras {options}, cal é a palabra máis comunmente asociada con "{category}"?'
        self.second_template = \
            'Cal é a palabra máis relacionada ca palabra "{category}" das palabras {options}?'
        self.third_template = \
            'Das seguintes palabras, escolle a palabra máis comunmente asociada ca palabra "{category}" - {options}:'
