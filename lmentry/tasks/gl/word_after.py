...
from lmentry.scorers.gl.word_after_scorer import WordAfterScorer
...
class WordAfter(LMentryTask):
    ...
    def __init__(self, name="word_after"):
        ...
        self.canonical_template = 'Na oración "{sentence}", que palabra vai despois da palabra "{word}"?'
        self.second_template = 'Na oración "{sentence}", que palabra é a inmediatamente posterior á palabra"{word}"?'
        self.third_template = 'Que palabra vai despois de "{word}" na oración "{sentence}"?'
