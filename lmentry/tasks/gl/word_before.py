...
from lmentry.scorers.gl.word_before_scorer import WordBeforeScorer
...
class WordBefore(LMentryTask):
    ...
    def __init__(self, name="word_before"):
        ...
        self.canonical_template = 'Na oración "{sentence}", que palabra vai antes da palabra "{word}"?'
        self.second_template = 'Na oración "{sentence}", que palabra é a inmediatamente anterior á palabra"{word}"?'
        self.third_template = 'Que palabra vai antes de "{word}" na oración "{sentence}"?'
