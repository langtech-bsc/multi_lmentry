...
from lmentry.scorers.gl.starts_with_word_scorer import StartsWithWordScorer
...
class StartsWithWord(LMentryTask):
    ...
    def __init__(self, name="starts_with_word"):
        super().__init__(name)
        self.canonical_template = 'Escribe unha oración que comece ca palabra "{word}":'
        self.second_template = 'Escribe unha oración cuxa primeira palabra sexa "{word}":'
        self.third_template = 'Escribe unha oración que comece con "{word}":'
    ...
    def create_data(self, num_examples=1000, seed=None, task_data_path=None):
        ...
        for template_id, template in enumerate(self.all_templates):
            for word, word_info in task_words.items():
                ...
                # create the example
                chosen_cats = ["ADPOSITION", "DETERMINER", "DATE", "VERB"]
