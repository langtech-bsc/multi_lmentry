...
class AllWordsFromCategory1Distractors(LMentryTask):
    ...
    def __init__(self, name="all_words_from_category_1_distractors"):
        ...
        self.canonical_template = 'Totes les paraules {words} són {category}? Contesta exactament "sí" o "no".'
        self.second_template = 'Totes les paraules {words} fan referència a {category}? Contesta exactament "sí" o "no".'
        self.third_template = 'La llista [{words}] conté exclusivament {category}? Contesta exactament "sí" o "no".'
    ...
    def _create_input(self, template, words, category):
        ...
        if template_id != 2:  # add "and" before the last word
            words = ", ".join(words[:-1]) + f" i {words[-1]}"
        else:
            words = ", ".join(words)
