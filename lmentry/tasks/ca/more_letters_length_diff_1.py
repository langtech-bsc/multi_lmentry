...
class MoreLettersLengthDiff1(LMentryTask):
    ...
    def __init__(self, name="more_letters_length_diff_1"):
        ...
        self.canonical_template = 'Quina paraula té més lletres: "{word1}" o "{word2}"?'
        self.second_template = 'Quina paraula és més llarga: "{word1}" o "{word2}"?'
        self.third_template = 'De les paraules "{word1}" i "{word2}", quina té més lletres?'
        self.all_templates = [self.canonical_template, self.second_template, self.third_template]
        self.parameter_names = ["answer", "distractor"]
        self.num_to_word = dict(
            zip(range(2, 11), ["dos", "tres", "quatre", "cinc", "sis", "set", "vuit", "nou", "deu"])
        )
