...
from lmentry.scorers.pt_br.scorer import LMentryScorer, the_word_regex
...
class StartsWithWordScorer(LMentryScorer):
    def __init__(self):
        ...
        self.prefixes = [
            r"La frase( es)?",
            r"{a_} frase que empieza con ( la palabra)? {word} ( es)?",
            r"{a_} frase que empieza con ( la palabra)? {word}( es)?",
            r"{a_} frase cuya primera palabra sea {word}( es)?",
        ]
        # we don't want an empty suffix here
        self.suffixes = [
            r"empieza con {word}",
            r"es una frase que empieza con {word}",
            r"es una frase que comienza con {word}",
            r"es una frase cuya primera letra es {word}",
        ]
