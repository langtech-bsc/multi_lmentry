...
class WordNotContainingScorer(LMentryScorer):
    ...
    def get_base_patterns(self, letter, word):
        ...
        contains = r"(contiene|tiene|incluye)"

        base_patterns = [
            rf"{word} (es una palabra que )?no {contains} (la letra )?{letter}",
            rf"Una palabra que no {contains} {letter} es {word}",
            rf"{word} es una palabra sin (la letra )?{letter}",
        ]
    ...
    def score_prediction(self, prediction, example, truncate_prediction: bool = False):
        ...
        if letter != "u":
            allowed_word += r"|(u)"
        if letter != "a" or letter != "y" or letter != "e" or letter != "o" or letter != "u":
