...
class EndsWithWordScorer(LMentryScorer):
    def __init__(self):
        ...
        self.prefixes = [
            r"A frase ( é)?(:)?",
            r"{a_} frase que termina com ( a palavra)? {word} ( é)?",
            r"{a_} frase que acaba com ( a palavra)? {word}( é)?",
            r"{a_} frase cuja {last_} palavra seja {word}( es)?",
        ]
    ...
    def get_base_patterns(self, word):
        ...
        # we want a word to contain at least two letters, with some exceptions.
