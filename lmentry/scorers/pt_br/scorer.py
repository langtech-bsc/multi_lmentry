...
word_regex_template = r'(A palavra )?["\'\.,]*\b{}\b["\'\.,]*'
sentence_regex_template = r'((A frase )?["\'\.,]*{}["\'\.,]*|A frase)'
letter_regex_template = r'(A letra )?["\'\.,]*\b{}\b["\'\.,]*'
number_regex_template = r'(O número )?["\'\.,]*\b{}\b["\'\.,]*'
...
def the_words_regex(words: list[str]):
    ...
    return rf"(As palavras )?{words_regex}"
...
def the_list_regex(words: list[str]):
    ...
    return rf"(((A lista )?{words_as_string})|a lista)"
...
class LMentryScorer:
    ...
    def get_shared_patterns(target):
        ...
        patterns = [
            rf"{target} é a resposta",
            rf"{target} é a resposta (correta|certa)",
            rf"A resposta é {target}",
            rf"A resposta (correta|certa) é {target}",
            rf"Resposta: {target}",
            rf"Resposta (correta|certa): {target}",
            rf"Resposta: {target}"
        ]
    ...
    def certainty_scorer(self, prediction, base_patterns):
        ...
        for i, pattern in enumerate(self.get_patterns(base_patterns)):
            try:
                ...
                print(f"Error in `certainty_scorer` with base_patterns = {base_patterns} / pattern = {pattern} / prediction = {prediction}.") # TODO
                raise err
