...
from lmentry.scorers.pt_br.scorer import LMentryScorer, the_sentence_regex, the_word_regex
...
class FirstWordScorer(LMentryScorer):
    def __init__(self):
        ...
        self.suffixes.extend([
            " (de|em|da frase) {sentence}",
        ])

    def get_base_patterns(self, answer, sentence):

        base_patterns = [
            rf"A primeira palavra é {answer}",
            rf"A primeira palavra (de|em|da frase) {sentence} é {answer}",
            rf"A palavra inicial (de|em|da frase) {sentence} é {answer}",
            rf"{answer} é a primeira palavra",
            rf"{answer} é a palavra inicial",
            rf"{sentence} começa com {answer}",
            rf"{sentence} começa com a palavra {answer}",
            rf"Primeira palavra: {answer}",
        ]
