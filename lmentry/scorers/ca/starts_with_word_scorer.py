...
class StartsWithWordScorer(LMentryScorer):
    def __init__(self):
        ...
        self.prefixes = [
            r"La frase ( és)?(:)?",
            r"{a_} frase que comença amb ( la paraula)? {word} ( és)?",
            r"{a_} frase la primera paraula de la qual és {word}( és)?",
        ]
        # we don't want an empty suffix here
        self.suffixes = [
            r"comença amb {word}",
            r"és una frase que comença amb {word}",
            r"és una frase la primera paraula de la qual és {word}",
        ]
