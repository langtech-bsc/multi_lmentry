...
class WordNotContainingScorer(LMentryScorer):
    ...
    def get_base_patterns(self, letter, word):
        ...
        contains = r"(conté|inclou|té)"

        base_patterns = [
            rf"{word} (és una paraula que )? no {contains} (la lletra )? {letter}",
            rf"Una paraula que no {contains} (la lletra )? {letter} és {word}",
            rf"{word} és una paraula sense (la lletra )? {letter}",
        ]
    ...
    def score_prediction(self, prediction, example, truncate_prediction: bool = False):
        ...
        if letter != "o":
            allowed_word += r"|(o)"
        if letter != "a" or letter != "i" or letter != "o":
