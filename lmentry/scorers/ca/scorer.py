...
word_regex_template = r'(La paraula )?["\'\.,]*\b{}\b["\'\.,]*'
sentence_regex_template = r'((La frase )?["\'\.,]*{}["\'\.,]*|La frase)'
letter_regex_template = r'(La lletra )?["\'\.,]*\b{}\b["\'\.,]*'
number_regex_template = r'(El nombre )?["\'\.,]*\b{}\b["\'\.,]*'
...
def the_words_regex(words: list[str]):
    ...
    return rf"(Les paraules )?{words_regex}"
...
def the_list_regex(words: list[str]):
    ...
    return rf"(((La llista )?{words_as_string})|la llista)"
...
class LMentryScorer:
    ...
    def get_shared_patterns(target):
        ...
        patterns = [
            rf"{target} és la resposta",
            rf"{target} és la resposta correcta",
            rf"La resposta és {target}",
            rf"La resposta correcta és {target}",
            rf"Resposta: {target}",
            rf"Resposta correcta: {target}"
        ]
