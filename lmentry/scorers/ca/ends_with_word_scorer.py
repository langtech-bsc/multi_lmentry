...
class EndsWithWordScorer(LMentryScorer):
    def __init__(self):
        ...
        a_ = r"(una)"
        last_ = r"(l'última|la darrera)"

        # we don't want an empty prefix here
        self.prefixes = [
            r"La frase ( és)?(:)?",
            r"{a_} frase que acaba amb ( la paraula)? {word} ( és)?",
            r"{a_} frase acabada en {word}( és)?",
            r"{a_} frase {last_} paraula de la qual és {word}( és)?",
        ]
    ...
    def get_base_patterns(self, word):
        ...
        # we want a word to contain at least two letters, with the exception of "a" and "i" (in catalan, also "o")
        valid_word = r"(a|i|o|\w\w+)"
