...
from lmentry.scorers.ca.scorer import LMentryScorer, swap_substrings, the_number_regex
...
class SmallerNumberScorer(LMentryScorer):
    ...
    def __init__(self):
        ...
        self.prefixes.extend([
            "Dels nombres {n1} i {n2}, ",
            "D'entre els nombres {n1} i {n2}, ",
            "Entre els nombres {n1} i {n2}, ",
        ])

    def get_base_patterns(self, answer, distractor):

        bigger = r"(major|més gran)"
        smaller = r"(menor|més petit)"

        base_patterns = [
            rf"{answer} és {smaller}",
            rf"{answer} és {smaller} que {distractor}",
            rf"{answer} és el nombre {smaller}",
            rf"El nombre {smaller} és {answer}",
        ]
