...
from lmentry.scorers.es.scorer import LMentryScorer, swap_substrings, the_number_regex
...
class SmallerNumberScorer(LMentryScorer):
    ...
    def __init__(self):
        ...
        self.prefixes.extend([
            "De los números {n1} y {n2}, ",
            "Entre los números {n1} y {n2}, ",
            "De entre los números {n1} y {n2}, ",
        ])

    def get_base_patterns(self, answer, distractor):

        bigger = r"(más grande|mayor)"
        smaller = r"(más pequeño|menor)"

        base_patterns = [
            rf"{answer} es {smaller}",
            rf"{answer} es {smaller} que {distractor}",
            rf"{answer} es el número {smaller}",
            rf"El número {smaller} es {answer}",
        ]
