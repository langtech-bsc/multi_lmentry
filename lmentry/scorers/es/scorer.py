...
word_regex_template = r'(La palabra )?["\'\.,]*\b{}\b["\'\.,]*'
sentence_regex_template = r'((La frase )?["\'\.,]*{}["\'\.,]*|La frase)'
letter_regex_template = r'(La letra )?["\'\.,]*\b{}\b["\'\.,]*'
number_regex_template = r'(El número )?["\'\.,]*\b{}\b["\'\.,]*'
...
def the_words_regex(words: list[str]):
    ...
    return rf"(Las palabras )?{words_regex}"
...
def the_list_regex(words: list[str]):
    ...
    return rf"(((La lista )?{words_as_string})|la lista)"
...
class LMentryScorer:
    ...
    def get_shared_patterns(target):
        ...
        patterns = [
            rf"{target} es la respuesta",
            rf"{target} es la respuesta correcta",
            rf"La respuesta es {target}",
            rf"La respuesta correcta es {target}",
            rf"Respuesta: {target}",
            rf"Respuesta correcta: {target}"
        ]
