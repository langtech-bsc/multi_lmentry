...
from lmentry.scorers.es.scorer import LMentryScorer, the_word_regex, the_words_regex, swap_substrings
...
class LeastAssociatedWordScorer(LMentryScorer):
    ...
    def get_base_patterns(self, answer, category, words):
        ...
        least_associated = r"menos (asociada a|asociada con|relacionada con)"

        base_patterns = [
            rf"La palabra {least_associated} {category} es {answer}",
            rf"La palabra {least_associated} {category} entre {words} es {answer}",
            rf"La categoría {category} es la {least_associated} {answer}",
            rf"{answer} es la palabra {least_associated} {category}",
            rf"{answer} es la {least_associated} {category}",
            rf"La palabra menos relacionada con {category} es {answer}",
        ]
    ...
    def score_prediction(self, prediction, example, truncate_prediction: bool = False):
        ...
        of = r"(de|entre)"
        options = r"(palabras|opciones)"
        given = r"(indicadas|listadas|proporcionadas|mencionadas|dadas)"
        words = rf"{of} ({words}|las {options} {given})"
