...
from lmentry.scorers.ko.scorer import LMentryScorer, swap_substrings, the_number_regex
...
class SmallerNumberScorer(LMentryScorer):
    ...
    def get_base_patterns(self, answer, distractor):
        ...
        base_patterns = [
            rf"{answer}{is_} (더 작다|작다)",
            rf"{answer}{is_} {distractor} 보다 (더 작다|작다)",
            rf"{answer}{is_} 가장 작다",
            rf"가장 작 숫자는 {answer}(이다|다)",
        ]
