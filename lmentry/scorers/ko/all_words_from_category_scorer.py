...
class AllWordsFromCategoryScorer(LMentryScorer):
    ...
    def get_base_patterns(self, answer, category):
        ...
        all_words = r"(다음 단어들은 모두|다음 단어는 모두|주어진 모든 단어는|주어진 모든 것은)"
        listed = r"(나열된|리스트에 있는)"
        belong = r"(대표한다|에 해당한다)"

        base_patterns = [
            rf"{answer}, 주어딘 리스트는 {category}에 해당한다. ",
    ...
    def negative_scorer(prediction, answer):
        ...
        opposite_answer = "아니요" if answer == "예" else "예"
    ...
    def score_prediction(self, prediction, example, truncate_prediction: bool = False):
        ...
        answer = "예" if metadata["num_distractors"] == 0 else "아니요"
