...
class WordNotContainingScorer(LMentryScorer):
    def __init__(self):
        ...
        # 'Idatzi "{letter}" letra ez duen hitz bat:\n'
        # 'Idatzi hitz bat "{letter}" letra ez daukana:\n'
        # 'Idatzi hitz bat zeinetan "{letter}" letra ez den agertzen:\n'

    def get_base_patterns(self, letter, word):

        that_has_not = r"ez (daukan|duen)"
        letter = rf"\"?{letter}\"?"

        base_patterns = [
            rf"{letter}( letra)? {that_has_not} hitz bat {word} da",
            rf"{letter}( letra)? {that_has_not} hitz bat da {word}",
            rf"{word} {letter}( letra)? {that_has_not} hitz bat da",
            rf"{letter}( letra)? {that_has_not} hitz bat: {word}",
            rf"hitz bat zeinetan {letter}( letra)? ez den agertzen: {word}",
            rf"hitz bat zeinetan ez den agertzen {letter}( letra)?: {word}",
            rf"{word} hitzean {letter}( letra)? ez da agertzen",
            rf"{letter}( letra)? ez da agertzen {word} hitzean",
            rf"{word} hitzean ez da {letter}( letra)? agertzen",
            rf"ez da {letter}( letra)? agertzen {word} hitzean",
            rf"{word} hitzak ez (du|dauka) {letter} letra",
        ]
