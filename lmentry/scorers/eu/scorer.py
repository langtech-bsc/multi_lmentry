...
from lmentry.eu_num2words import num2words

logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%Y/%m/%d %H:%M:%S', level=logging.INFO)

word_regex_template = r'["\'.,]*\b{}\b["\'.,]*( hitz[ae]k?)?'
sentence_regex_template = r'(["\'.,]*{}["\'\.,]*( esaldi[ea]k?)?|Esaldi[ea]k?)'
letter_regex_template = r'["\'.,]*\b{}\b["\'.,]*( letr[ae]k?)?'
number_regex_template = r'["\'.,]*\b{}\b["\'.,]*( zenbaki[ae]k?)?'


def standardized_number_regex(number: int):
    digits_string = str(number)
    word_form = num2words(number)
    return fr'(\b{digits_string}\b|{word_form})'
...
def the_words_regex(words: list[str]):
    ...
    words = words[:-1] + ["eta"] + words[-1:]
    words = [rf"{word}{W}+" for word in words[:-1]] + [words[-1]]
    words[-2] = rf"({words[-2]})?"
    words = [rf"{W}*"] + words + [rf"{W}*"]
...
def the_list_regex(words: list[str]):
    ...
    return rf"(({words_as_string}( esaldi[ae]k?)?)|Esaldi[ae]k?)"
...
class LMentryScorer:
    ...
    def get_shared_patterns(target):
        ...
        patterns = [
            rf"{target} da erantzuna",
            rf"Erantzuna {target} da",
            rf"Erantzun zuzena {target} da",
            rf"Erantzuna: {target}"
        ]
