...
word_regex_template = r'(La parola )?["\'\.,]*\b{}\b["\'\.,]*'
sentence_regex_template = r'((La frase )?["\'\.,]*{}["\'\.,]*|La frase)'
letter_regex_template = r'(La lettera )?["\'\.,]*\b{}\b["\'\.,]*'
number_regex_template = r'(Il numero )?["\'\.,]*\b{}\b["\'\.,]*'


def standardized_number_regex(number: int):
    digits_string = str(number)
    word_form = num2words(number)
    return rf'(\b{digits_string}\b|{word_form}|{word_form.replace("-", " ")})'
...
def the_words_regex(words: list[str]):
    ...
    return rf"(Le parole )?{words_regex}"
...
def the_list_regex(words: list[str]):
    ...
    return rf"(((L'elenco )?{words_as_string})|l'elenco)"
...
class LMentryScorer:
    ...
    def normalize_prediction(prediction: str, truncate_prediction: bool = False):
        ...
        prediction = prediction.replace(
            "_", " "
        )  # this is an easy hack to not have \w include "_"
    ...
    def get_shared_patterns(target):
        ...
        patterns = [
            rf"{target} è la risposta",
            rf"{target} è la risposta corretta",
            rf"{target} è la risposta giusta",
            rf"La risposta è {target}",
            rf"La risposta corretta è {target}",
            rf"La risposta giusta è {target}",
            rf"Risposta: {target}",
            rf"Risposta corretta: {target}",
            rf"Risposta giusta: {target}",
        ]
    ...
    def score_predictions(
        ...
        for id_ in predictions:
            ...
            score, certainty = self.score_prediction(
                prediction, example, truncate_predictions
            )
    ...
    def _simple_scorer(
        self, prediction, answer, allow_answer_pattern_repetitions=True
    ) -> Tuple[int, int]:
        ...
        if not re.search(
            r"[^\W_]", prediction
        ):  # we don't consider `_` to be alphanumeric
            return 0, 1
    ...
    @staticmethod
    def display_prediction_patterns(
        task, model_name, show_only_uncertain_scores: bool = True
    ):
        # load the predictions
        predictions_path = task.predictions_dir.joinpath(model_name).with_suffix(
            ".json"
        )
        with open(predictions_path) as f_predictions:
            predictions = json.load(f_predictions)
        ...
        for id_, prediction_entry in predictions.items():
            ...
            for (
                ...
                task.parameter_names
            ):  # currently doesn't support parameters that are sequences
